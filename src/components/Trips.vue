      ...
      <template v-slot:top-right>
        ...
        <template v-if="!disable" v-slot:body-cell-lrNumber="props">
          <q-td key="lrNumber" :props="props">
            {{ props.row.lr_number }}
            <q-popup-edit @save="updatelrNumber(props.row)" v-model="props.row.lr_number" :disable="isEditableLRNumber(props.row.lr_number)">
         ...
         <template v-if="!disable" v-slot:body-cell-unloadQty="props">
          <q-td key="unloadQty" :props="props">
            ...
            <q-popup-edit @save="updateUnloadedQty(props.row)" v-model.number="props.row.unloaded_quantity" :disable="isEditableUnloadedQuantity(props.row.unloaded_quantity)">
...
function shortageAmount (t) {
  if (!t.permit || !t.permit.owner_shortage_percentage) {
    return 0
  }
  ...
  if (shortagePercentage >= t.permit.owner_shortage_percentage) {
    amount = shortage * t.permit.owner_shortage_per_tonne
  }
...
function rate (v) { return v.permit ? v.permit.owner_rate_per_tonne : 0 }

function calculateFuelPrice (fuel) {
  let price = 0
  if (fuel.rate && fuel.quantity) {
    price = fuel.rate * fuel.quantity
  } else if (fuel.total) {
    price = fuel.total
  }
  console.log(price)
...
function totalFuel (tp) {
  ...
  for (let i = 0; i < fuelData.length; i++) {
    if (fuelData[i].rate && fuelData[i].quantity) {
      totalFuelPrice = totalFuelPrice + fuelData[i].rate * fuelData[i].quantity
    } else if (fuelData[i].total) {
      totalFuelPrice = totalFuelPrice + fuelData[i].total
    }
  }
...
export default {
  ...
  created () {
    ...
    if (!this.visibleColumnsProp) {
      this.visibleColumns = ['passNumber', 'passDate', 'truckNumber', 'lrNumber', 'loadQty', 'unloadQty', 'advance', 'fuel', 'deduction']
  ...
  data () {
    return {
      ...
      fuelColumns: [
        ...
        { name: 'amount', field: v => calculateFuelPrice(v), label: 'Amount', align: 'left', format: v => v.toFixed(2) }
      ],
      newFuel: {},
      advanceData: [],
      newAdvance: {
      ...
      tableData: {
        columns: [
          { name: 'passNumber', label: 'Pass Number', field: 'tp_number', align: 'left', sortable: true },
          { name: 'passDate', field: 'tp_date', label: 'Pass Date', align: 'left', format: v => moment(v).format('DD/MM/YYYY') },
  ...
  methods: {
    ...
    isEditableLRNumber (lrNumber) {
      if (this.isAdmin() || this.isVerifier() || this.isAuthorizer()) {
        return false
      } else if (lrNumber) {
        return true
      } else {
    ...
    isEditableUnloadedQuantity (unloadedQuantity) {
      if (this.isAdmin() || this.isVerifier() || this.isAuthorizer()) {
        return false
      } else if (unloadedQuantity) {
        return true
      } else {
        return false
      }
    ...
    filterInput (rows, terms, cols, getCellValue) {
      ...
      return rows.filter(row => row.truck_number.includes(this[terms]) ||
        ...
        row.tp_number.includes(this[terms]))
    },
    ...
    editTPAdvance (tpDetails) {
      ...
      if ((!(this.isAdmin() || this.isVerifier() || this.isAuthorizer())) && this.advanceData.length) {
        return
      }
    ...
    editTPFuel (tpDetails) {
      ...
      if ((!(this.isAdmin() || this.isVerifier() || this.isAuthorizer())) && this.fuelData.length) {
        return
      }
