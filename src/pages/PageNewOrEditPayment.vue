...
export default {
  ...
  async created () {
    ...
    if (this.payment != null) {
      ...
      console.log(this.permit, this.order, this.type)
  ...
  apollo: {
    ...
    trips: {
      skip () {
        return !this.permit.length || this.disablePermits || !this.owner
      },
  ...
  methods: {
    ...
    async updateVoucher () {
      try {
        ...
        let response = await this.$apollo.mutate({
          mutation: updateString('payment_vouchers', `
            affected_rows
            returning {
              id
            }
    ...
    async onSubmit () {
      ...
      try {
        let response = await this.$apollo.mutate({
          mutation: insertString('payment_vouchers', `
            affected_rows
            returning {
              id
            }
          ...
          variables: {
            ...
            on_conflict: {
              constraint: 'payment_vouchers_pkey',
        ...
        const paymentVoucherId = response.data.insert_payment_vouchers.returning[0].id
