 ...
 <div class="q-pt-md">
      ...
      <div style="max-width: 250px">
        <q-file
          ...
          v-model="path"
          accept=".xlsx, .xls"
          label="Add/Update Vendors(Excel)"
        />
      </div>
      <q-table
      class="q-pt-md"
...
export default {
  ...
  methods: {
    getSelectedString () {
      return this.selected.length === 0 ? '' : `${this.selected.length} truck${this.selected.length > 1 ? 's' : ''} selected of ${this.vendors.length}`
    },
    ...
    async onFileSelected (value) {
      ...
      this.vendors = XLSX.utils.sheet_to_json(workbook.Sheets[sheet[0]], {
        range: 1,
        raw: true,
        defval: null,
        header: [ 'name', 'gstn_pan', 'phone_number', 'gst_tds', 'address1', 'address2', 'city', 'state', 'pincode' ]
      })
    ...
    filterInput (rows, terms, cols, getCellValue) {
      ...
      return rows.filter(row =>
        ...
        row.gstn_pan.toLowerCase().includes(terms) ||
    ...
    async save () {
      ...
      try {
        const response = await this.$apollo.mutate({
          mutation: insertString('vendors', `
            affected_rows
          `, true),

          variables: {
            objects: this.vendors.map(t => ({
              ...t,
              companies_bridge: {
                data: {
        ...
        console.error(ex)
        this.$q.notify({
          message: 'Failed to save, some vendors not exist',
          color: 'positive',
          icon: 'info'
        })
      }
    ...
    removeSelected () {
      ...
      this.$q.dialog({
        ...
        try {
          this.vendors = _.difference(this.vendors, this.selected, v => v.gstn_pan)
