          ...
          <table class="q-table q-pl-md q-pr-md q-pt-lg">
            <thead>
              <tr>
                ...
                <th>Shortage Allowance</th>
                <th>Shortage Charge</th>
                <th>Line Total</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in items" :key="item.name">
                <td>
                  <q-input
                  :rules="[val => !!val || 'This Field is required']"
                   style="width: 150px" outlined type="text" v-model="item.name"></q-input>
                </td>
                <td>
                  <q-input
                    :rules="[val => !!val || 'This Field is required']"
                ...
                <td>
                  <q-input
                    ...
                    outlined
                    type="float"
                    v-model="item.shortageAllowance"
                  ></q-input>
                </td>
...
export default {
  ...
  mixins: [
    ...
    TPS({
      skip () {
        return !this.account || !this.client || !this.formData.permit_number
      },
      variables () {
        const obj = {
          limit: 50,
          where: {
            permit_id: {
              _eq: this.permits.find(p => p.permit_number === this.formData.permit_number)['id']
            },
            unloaded_quantity: {
              _gt: 0.0
            }
  ...
  created () {
    if (this.permit) {
      ...
      this.items = []
      this.orderOrPermit = this.permit
    }
  ...
  computed: {
    ...
    permitsAndOrders () {
      let orderData = this.permits.length ? this.permits.map(order => {
        return {
          id: order.id,
          value: order.permit_number,
          type: 'order'
        }
      }) : []
    ...
    tpOptions () {
      let order = this.permits.find(p => p.permit_number === this.formData.permit_number)
      let tripData = this.trips.length ? this.trips.map(tp => {
        return {
          id: tp.tp_number,
          unitCost: order['client_rate_per_tonne'],
          loadedQty: tp.load_carrying,
          unloadedQty: tp.unloaded_quantity,
          shortageAllowance: order['client_shortage_percentage'],
          shortageCost: order['client_shortage_per_tonne'],
          value: tp.tp_number,
          type: 'order'
        }
      }) : []
  ...
  methods: {
    ...
    onPermitOrOrderSelected (value) {
      ...
      // if (type === 'order') {
      //   let order = this.orders.find(p => p.order_number === value)
      //   console.log(order['client_rate_per_tonne'], order['quantity'])
      // }
      // if (type === 'order') {
      //   let order = this.permits.find(p => p.permit_number === value)
      //   console.log(order['client_rate_per_tonne'], order['quantity'])
      // }
      // this.TPS()
    },
    ...
    onTPSelected (val) {
      ...
      console.log(val)
      this.$refs.tpSelect.updateInputValue('', true)

      this.items = []

      this.tpsSelected.forEach(tp => {
        this.items.push(
          {
            name: tp.value,
            unitCost: tp.unitCost,
            loadedQty: tp.loadedQty,
            unloadedQty: tp.unloadedQty,
            shortageAllowance: tp.shortageAllowance,
            shortageCost: tp.shortageCost
          }
        )
      })
    },
    ...
    async createInvoice () {
      ...
      invoice.quantity = 0 // this.items[0].quantity
