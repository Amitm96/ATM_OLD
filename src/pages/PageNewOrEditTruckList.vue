<template>
 ...
 <div class="q-pt-md">
     <q-input
      ...
      <template v-slot:top>
        <q-btn v-if="trucklist" color="primary" :disable="loading" label="New Truck" :to="{name: 'newtruck', params: {trucklist: trucklist}}" />
...
export default {
  ...
  async created () {
    console.log('route params', this.trucklist, this.id, this.$route.path)
    if (!this.trucklist && this.id) {
      const response = await getObjectById(this.$apollo, 'truck_lists', `
        id
        name
      `, this.id)
      ...
      this.trucklist = response.data.truck_lists[0]
    }

    if (this.trucklist) {
      this.name = this.trucklist.name
    }
  },
  ...
  apollo: {
    trucks: {
      ...
      variables () {
        const obj = {
          where: { truck_lists: { truck_list_id: { _eq: this.trucklist.id } } }
        }
  ...
  methods: {
    ...
    filterInput (rows, terms, cols, getCellValue) {
      ...
      return rows.filter(row => row.truck_number.includes(terms) ||
        ((row.ownerName || (row.owner && row.owner.name)) || '').includes(terms) ||
        ((row.ownerPhoneNumber || (row.owner && row.owner.phone_number)) || '').includes(terms))
    },
      ...
      this.$q.dialog({
        ...
        try {
          await this.$apollo.mutate({
            mutation: deleteString('truck_lists_bridge', `
              affected_rows
            `),
            variables: {
              where: {
