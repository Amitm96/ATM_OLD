<template>
  <data-table
    ...
    collection="vendor_payment_vouchers"
    :columns="columns"
    newlabel="New Payment Voucher"
    newrow="/newpaymentvendor"
    editrow="editpaymentvendor"
    editprop="payment"
    rowkey=""
    :searchField="false"
    :query="query"
...
import { listQuery, vendorPaymentFields } from '../utils/gql'
...
const columns = [
  ...
  },
  {
    ...
    align: 'left',
    label: 'Vendor Name',
    field: p => p.vendor.name
  },
  {
    name: 'phoneNumber',
    align: 'left',
    label: 'Phone Number',
    field: p => p.vendor.phone_number
  },
  {
    name: 'quantity',
    align: 'left',
    ...
    actions: [
      {
        icon: 'subject',
        to (props) {
          return { name: 'paymentvouchervendor', params: { payment: props.row } }
        },
...
export default {
  name: 'VendorPaymentsList',

  data () {
    return {
      columns,
      query: listQuery('vendor_payment_vouchers', vendorPaymentFields)
    }
  },
